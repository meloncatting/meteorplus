...
public class GuiRenderer {
    ...
    public boolean renderTooltip(GuiGraphicsExtractor graphics, double mouseX, double mouseY, double delta) {
        tooltipAnimProgress += (tooltip != null ? 1 : -1) * delta * 16;
    ...
    public void roundedRect(double x, double y, double w, double h, double radius, Color color) {
        quad(x + radius, y, w - 2 * radius, h, color);
        quad(x, y + radius, radius, h - 2 * radius, color);
        quad(x + w - radius, y + radius, radius, h - 2 * radius, color);
        int steps = 8;
        double[] cx = {x + radius, x + w - radius, x + w - radius, x + radius};
        double[] cy = {y + radius, y + radius, y + h - radius, y + h - radius};
        double[] sa = {Math.PI, 3.0 * Math.PI / 2.0, 0.0, Math.PI / 2.0};
        for (int c = 0; c < 4; c++) {
            double ccx = cx[c], ccy = cy[c], angle = sa[c];
            for (int i = 0; i < steps; i++) {
                double a1 = angle + (Math.PI / 2.0) * i / steps;
                double a2 = angle + (Math.PI / 2.0) * (i + 1) / steps;
                triangle(ccx, ccy,
                    ccx + radius * Math.cos(a1), ccy + radius * Math.sin(a1),
                    ccx + radius * Math.cos(a2), ccy + radius * Math.sin(a2),
                    color);
            }
        }
    }
