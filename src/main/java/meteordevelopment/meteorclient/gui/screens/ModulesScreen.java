...
public class ModulesScreen extends TabScreen {
    ...
    private WSearchOverlay searchOverlay;
    ...
    public void initWidgets() {
        ...
        searchOverlay = theme.searchOverlay();
        if (searchOverlay != null) add(searchOverlay);
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        if (searchOverlay != null && searchOverlay.isOpen()) {
            double s = mc.getWindow().getGuiScale();
            if (!searchOverlay.isOver(click.x() * s, click.y() * s)) {
                searchOverlay.close();
            }
        }
    ...
    public boolean keyPressed(KeyEvent value) {
        ...
        boolean ctrl = MacosUtil.IS_MACOS ? value.modifiers() == GLFW_MOD_SUPER : value.modifiers() == GLFW_MOD_CONTROL;

        if (ctrl && (value.key() == GLFW_KEY_K || value.key() == GLFW_KEY_F)) {
            if (searchOverlay != null) searchOverlay.open();
            return true;
        }

        if (searchOverlay != null && searchOverlay.isOpen()) {
            return switch (value.key()) {
                case GLFW_KEY_ESCAPE -> { searchOverlay.close(); yield true; }
                case GLFW_KEY_UP -> { searchOverlay.navigate(-1); yield true; }
                case GLFW_KEY_DOWN -> { searchOverlay.navigate(1); yield true; }
                case GLFW_KEY_ENTER, GLFW_KEY_KP_ENTER -> { searchOverlay.toggleSelected(); yield true; }
                case GLFW_KEY_TAB -> true;
                default -> super.keyPressed(value);
            };
        }
